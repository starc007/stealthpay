...
export function ScanSweep() {
  ...
  const sweepTo: Address | undefined = destination.match(/^0x[a-fA-F0-9]{40}$/)
  ...
  const handleUnlockAndScan = async () => {
    ...
    try {
      ...
      const result = generateStealthKeysFromSignature(
  ...
  const sweepOne = async (
    ...
    const sweepAmount =
      payment.balance > gasReserve ? payment.balance - gasReserve : 0n;
    if (sweepAmount === 0n) return null;

    const stealthAccount = privateKeyToAccount(
      payment.stealthPrivKey as `0x${string}`,
    );
  ...
  const handleSweepAll = async () => {
    ...
    const sweepable = payments.filter(
      (p) => p.balance > 10000n && !p.sweepTxHash,
    );
  ...
  const sweepableCount = payments.filter(
    (p) => p.balance > 10000n && !p.sweepTxHash,
  ).length;
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-[10px] text-muted">
              ...
              {payment.stealthAddress.slice(-6)}
            </span>
          </div>
